...
def compute_pr_a(opts, max_real, num_gen, nhood_size, row_batch_size, col_batch_size):
    ...
    real_features = metric_utils.compute_feature_stats_for_dataset_OC(
        opts=opts, rel_lo=0, rel_hi=0, capture_all=True, max_items=max_real).get_all_torch().to(torch.float16).to(opts.device)

    gen_features = metric_utils.compute_feature_stats_for_generator_OC(
        opts=opts, rel_lo=0, rel_hi=1, capture_all=True, max_items=num_gen).get_all_torch().to(torch.float16).to(opts.device)
