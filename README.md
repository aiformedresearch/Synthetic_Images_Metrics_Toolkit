...
To evaluate your generative model, you need to customize the `configs/config.py` script to specify:
- the metrics you want to compute;
- runtime configurations (running directory, number of synthetic images to generate for the computation of the metrics, ...)
- real dataset configurations
- generator configurations (defining the functions to load the pre-trained generator and to generate new samples)

We tested the repository on three generative models for 2D image synthesis:

➡️ [StyleGAN2-ADA](https://github.com/NVlabs/stylegan2-ada-pytorch), a state-of-the-art generative adversarial network ([config.py](configs/StyleGAN2ADA/config_stylegan2ada.py));

➡️ [PACGAN](https://github.com/MatteoLai/PACGAN), a custom generative adversarial network ([config.py](configs/PACGAN/config_PACGAN.py));

➡️ [Mediffusion](https://github.com/BardiaKh/Mediffusion), a diffusion model ([config.py](configs/Mediffusion/config_mediffusion.py)).
...
| :-----        | :-----: | :---------- |
| `fid` | Fr&eacute;chet inception distance<sup>[1]</sup> against the full dataset | [StyleGAN2-ADA](https://github.com/NVlabs/stylegan2-ada-pytorch)
| `kid` | Kernel inception distance<sup>[2]</sup> against the full dataset         | [StyleGAN2-ADA](https://github.com/NVlabs/stylegan2-ada-pytorch)
| `pr` | Precision and recall<sup>[3]</sup> againt the full dataset               | [StyleGAN2-ADA](https://github.com/NVlabs/stylegan2-ada-pytorch)
| `is_`       | Inception score<sup>[4]</sup> against the full dataset                              | [StyleGAN2-ADA](https://github.com/NVlabs/stylegan2-ada-pytorch)
| `prdc`    |  Precision, recall, density, and coverage<sup>[5]</sup>  against the full dataset                    | [prdc](https://github.com/clovaai/generative-evaluation-prdc)
| `pr_auth`    |  	$\alpha$-precision, 	$\beta$-recall, and authenticity<sup>[6]</sup> against the full dataset  | [evaluating-generative-models](https://github.com/vanderschaarlab/evaluating-generative-models)
